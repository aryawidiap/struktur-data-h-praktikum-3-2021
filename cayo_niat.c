...
AVLNode* _avl_createNode(char value[1001]) {
...
AVLNode* _search(AVLNode *root, char value[1001]) {
    while (root != NULL) {
...
AVLNode* _insert_AVL(AVL *avl,AVLNode* node, char value[1001]) {
...
bool avl_find(AVL *avl, char value[1001]) {
...
void avl_insert(AVL *avl, char value[1001]){
...
void inorder(AVLNode *root, int *index) {
    if (root) {
...
void inorderluv(AVLNode *root, int *index) {
    if (root) {
        inorderluv(root->left, index);
        if(*index > 0){
            printf("--<3--");
        }
        printf("%s", root->data);
        (*index)++;
        inorderluv(root->right, index);
    }
}
...
int main(){
    ...
    avl_init(&avlku);
    char str[1001];
    while(scanf("%1000s", str) != EOF){
        avl_insert(&avlku, str);
    }
    int index = 0;
    inorder(avlku._root, &index);
    index = 0;
    inorderluv(avlku._root, &index);
